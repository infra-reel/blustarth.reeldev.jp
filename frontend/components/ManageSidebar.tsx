...
export default function ManageSidebar() {
  ...
  return (
    <aside className="w-48 bg-navy border-r border-white/10 flex flex-col py-6 flex-shrink-0">
      <Link href="/" className="font-heading font-700 text-sm tracking-widest px-5 mb-8 text-gray-mid hover:text-white">
        ← SITE
      </Link>
      <nav className="flex flex-col gap-1 px-3">
        {LINKS.map(l => (
          <Link key={l.href} href={l.href}
            className={`font-heading font-600 text-xs tracking-widest px-3 py-2.5 rounded-sm transition-colors ${
