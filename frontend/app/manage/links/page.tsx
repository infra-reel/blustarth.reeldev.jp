...
export default function ManageLinks() {
  ...
  const load = () => authFetch('/api/links').then(r => r.json()).then(setItems).catch(() => {})
  useEffect(() => { load() }, [])
  ...
  const del = async (id: number) => { if (!confirm('削除しますか？')) return; await authFetch(`/api/links/${id}`, { method: 'DELETE' }); load() }
